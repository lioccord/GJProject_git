...
@interface MWGridCell () {
    ...
    SDPieProgressView *_loadingIndicator;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _loadingIndicator = [[SDPieProgressView alloc] initWithFrame:CGRectMake(0, 0, 60.0f, 60.0f)];
        //_loadingIndicator1.userInteractionEnabled = NO;
...
//        }
//        _loadingIndicator.trackTintColor = [UIColor blueColor];
//        _loadingIndicator.progressTintColor = [UIColor redColor];
        //_loadingIndicator.
...
- (void)selectionButtonPressed {
    ...
    if (_delegate) {
        if ([_delegate respondsToSelector:@selector(MWGridCellDidSelected)]) {
            [_delegate MWGridCellDidSelected];
        }
    }
...
- (void)showImageFailure {
    if (!_loadingError) {
        ...
        _loadingError.image = IMAGE(@"无图片背景");//[UIImage imageNamed:@"MWPhotoBrowser.bundle/images/ImageError.png"];

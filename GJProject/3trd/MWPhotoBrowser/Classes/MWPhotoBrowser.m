...
-(void)viewDidLayoutSubviews{
    ...
    if ([_loginUserId isEqualToString:_userId]) {
...
#pragma mark - NavBar
-(void)setNavigaterBar{
    
    UIButton *leftButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
    [leftButton setImage:[UIImage imageNamed:@"NavBar_back"] forState:UIControlStateNormal];
    ...
    NSString    * loginID = self.loginUserId;
    NSString * userID = self.userId;
    
    if (loginID  != nil && userID != nil) {
        
        if ([loginID isEqualToString:userID]){
    ...
    NSString * curStr = [Utils id2str:[user objectForKey:kImageForUserId]];
    NSString * llStr = app.loginDictionary[@"UserId"];
    
    if ([[NSString stringWithFormat:@"%@",curStr] isEqualToString:[NSString stringWithFormat:@"%@",llStr]]) {
        return;
    }
   else {
        [self createRightTopBar];
...
-(void )createRightTopBar{

    UIButton *rightButton = [[UIButton alloc] init];
    rightButton.frame = CGRectMake(0, 0, 44, 44);
    [rightButton setImage:[UIImage imageNamed:@"NavBar_report"] forState:UIControlStateNormal];
...
-(void)rightBtnAction:(UIButton *)button{
    ...
    [alertSheet addAction:[UIAlertAction actionWithTitle:@"举报" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        
        ReporterVC * nextView = [[ReporterVC alloc]initWithNibName:@"ReporterVC" bundle:nil];
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            ...
            self.title = [NSString stringWithFormat:@"%lu %@", (unsigned long)numberOfPhotos, @"张图片"];
        }
